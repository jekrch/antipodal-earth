...
import { MapPin, Globe2, Info, X, Navigation, Settings, ChevronDown, ExternalLink } from 'lucide-react';
...
const prehistoricMapOptions: PrehistoricMapOption[] = prehistoricMapOptionsRaw
  .map(option => ({
    ...option,
    ageMa: extractMaFromName(option.name), // Populate age for sorting and display
    slug: generateSlug(option.name),       // Create URL-friendly identifier
  }))
  .sort((a, b) => a.ageMa - b.ageMa);     // Ensure chronological order
...
const GlobeViewer: React.FC = () => {
  ...
  const expectAnimatedMoveRef = useRef(false); // True if the next POV change should be animated
  const activeInteractionGlobeRef = useRef<'main' | 'secondary' | null>(null); // Tracks which globe is actively being manipulated to prevent conflicting updates.
  ...
  useEffect(() => {
    const handleResize = () => {
      setTimeout(() => {
        ...
        if (secondaryContainerRef.current) {
          const rect = secondaryContainerRef.current.getBoundingClientRect();
          if (rect.width > 0 && rect.height > 0) setSecondaryDimensions({ width: rect.width, height: rect.height });
        }
      }, 50); 
    };
    ...
    if (secondaryContainerRef.current) resizeObserver.observe(secondaryContainerRef.current);
            ...
            <div className="w-full max-w-xl lg:max-w-xs">
              <div className="flex flex-row items-stretch gap-x-4 lg:flex-col lg:items-stretch lg:gap-x-0 lg:gap-y-6">
                <div className="flex-1 w-full lg:flex-none space-y-3 lg:space-y-4">
                  {/* Secondary / Antipodal / Historic Point Display - MOVED UP & LINK MOVED TO LABEL */}
                  ...
                  {/* Primary View Display - MOVED DOWN & LINK MOVED TO LABEL */}
                  <div className="group">
                    <div className="flex items-center justify-between mb-1 sm:mb-2">
                      <div className="flex items-center gap-2">
            ...
            <div className="mb-6 space-y-3">
              ...
              {secondaryGlobeMode === 'prehistoric_same_point' && selectedPrehistoricMap && (
                <div className="text-xs text-neutral-500 mt-2 space-y-1">
                  <div className="flex items-center gap-1"> <button onClick={(e) => handleAttributionClick(selectedPrehistoricMap.attribution, e)} className="cursor-pointer underline decoration-dotted decoration-neutral-400 hover:decoration-neutral-300 text-neutral-300 hover:text-neutral-100 flex items-center bg-transparent border-none p-0 text-xs"> <Info size={13} className="inline mr-1 flex-shrink-0" /> CC Attribution</button> </div> <p className="mt-1">Note: Prehistoric map accuracy varies. Locations are overlaid on ancient continental arrangements.</p>
                </div>
              )}
            </div>
